...
class DB {
    ...
    static function getConnection(){
        if(self::$conn == null){
            self::$conn = new \PDO("mysql:host=localhost;dbname=seoul;charset=utf8mb4", "root", "", [\PDO::ATTR_DEFAULT_FETCH_MODE => \PDO::FETCH_OBJ, \PDO::ATTR_ERRMODE => \PDO::ERRMODE_EXCEPTION]);
        }
        return self::$conn;
    }
    ...
    static function who($email){
        return self::fetch("SELECT * FROM users WHERE user_email = ? ", [$email]);
    }
}